...
const EVALUATOR_CONTROL_PROMPT = [
	...
	"Bun.$ runs its template through bash, so full bash syntax works: heredocs, `$(...)`, pipes, redirect chains, escaped parens. Never wrap the command in a `bash -c` string — that only adds a quoting layer. An interpolated value becomes one escaped argument; to splice shell text verbatim, interpolate an object `{ raw: text }`. A dollar-brace in the template is still JS interpolation: escape the dollar for shell parameter expansion.",
	"",
